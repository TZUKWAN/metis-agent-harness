...
def schema_repair_feedback(schema_errors: list[str]) -> dict[str, list[str]]:
...
def schema_repair_hints(schema_errors: list[str]) -> list[str]:
    return schema_repair_feedback(schema_errors)["hints"]


def schema_repair_hint_types(schema_errors: list[str]) -> list[str]:
    return schema_repair_feedback(schema_errors)["hint_types"]


def schema_repair_hint_details(schema_errors: list[str]) -> list[dict[str, Any]]:
    ...
    for raw_error in schema_errors:
        ...
        if "additional property not allowed" in lowered:
            hints.append(_hint(
                "remove_additional_property", path, "additionalProperties", error,
                f"Remove the unsupported argument at {path}.",
            ))
        elif "missing required property" in lowered:
            hints.append(_hint(
                "add_required_property", path, "required", error,
                f"Add the required argument {path}.",
            ))
        elif "less than minitems" in lowered:
            hints.append(_hint(
                "increase_array_items", path, "minItems", error,
                f"Provide enough array items for {path}; do not pass an empty array.",
            ))
            ...
            hints.append(_hint("fix_type", path, "type", error, f"Wrong type for {path}: {detail}. Check if you need a string, number, or array."))
            ...
            hints.append(_hint("increase_numeric_value", path, keyword, error, f"The value at {path} is too small. Use a larger number."))
            ...
            hints.append(_hint("reduce_numeric_value", path, keyword, error, f"The value at {path} is too large. Use a smaller number."))
        elif "does not match pattern" in lowered:
            hints.append(_hint("fix_string_pattern", path, "pattern", error, f"The value at {path} does not match the required format."))
        ...
        elif "minlength" in lowered:
            hints.append(_hint("fix_min_length", path, "minLength", error, f"The value at {path} must not be empty."))
    return hints
